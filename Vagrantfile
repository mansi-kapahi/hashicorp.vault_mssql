...
VV1='VAULT_VERSION='+'1.7.5+ent.hsm'  # VV1='' to Install Latest OSS
...
Vagrant.configure("2") do |config|
	...
	config.vm.provision "shell" do |s|
		 s.path = "#{sPTH}/0.install_commons.sh"
		# s.path = "#{sPTH}/1.install_docker.sh"
	end
	config.vm.provision "shell" do |s|
		# s.path = "#{sPTH}/0.install_commons.sh"
		s.path = "#{sPTH}/1.install_docker.sh"
...
SCRIPT
			...
			vault_node.vm.provision "file", source: "#{sPTH}/2.install_mssql_docker.sh", destination: "#{sHOME}/install_mssql_docker.sh"
			vault_node.vm.provision "shell", inline: "/bin/bash -c '#{sHOME}/install_mssql_docker.sh #{iCLUSTERA_N}'"
...
SCRIPT
			...
			if iX == 1 then
				...
				vault_node.vm.provision "file", source: "#{sPTH}/6.post_setup_vault_database_mssql.sh", destination: "#{sHOME}/post_setup_vault.sh"
				vault_node.vm.provision "shell", inline: "/bin/bash -c '#{sHOME}/post_setup_vault.sh'"
			else
